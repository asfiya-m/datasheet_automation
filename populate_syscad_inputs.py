...
def populate_syscad_inputs(master_file: BytesIO, streamtable_file: BytesIO) -> BytesIO:
    ...
    # Get common sheet (example: 'Tank')
    equipment_type = 'Tank'  # This could be parameterized later
    if equipment_type not in master_wb.sheetnames or equipment_type not in streamtable_wb.sheetnames:
        raise ValueError(f"Sheet '{equipment_type}' not found in one of the workbooks.")
